...
class LoadedLightGlue(nn.Module):
    default_conf = {
    ...
    def __init__(self, conf) -> None:
        ...
        self.loss_fn = NLLLoss(conf.loss)
         ...
         # Directly load model from your specific checkpoint
...
__main_model__ = LoadedLightGlue
...
#         if features is not None and features in self.features:
#             for k, v in self.features[features].items():
#                 setattr(conf, k, v)

#         # Load model from your checkpoint instead of URL
#         checkpoint_path = "/homes/tp4618/Documents/bitbucket/SuperGlueThesis/external/glue-factory/outputs/training/sp+lg_densehomography/checkpoint_best.tar"
#         print(torch.load(checkpoint_path, map_location="cpu").keys())
#         state_dict = torch.load(checkpoint_path, map_location="cpu")['state_dict']
#         # Adjust to correct subdictionary if needed or directly load
#         self.load_state_dict(state_dict, strict=False)
...
#     #     # Load model from checkpoint instead of URL
#     #     checkpoint_path = "/homes/tp4618/Documents/bitbucket/SuperGlueThesis/external/glue-factory/outputs/training/sp+lg_densehomography/checkpoint_best.tar"
#     #     print(torch.load(checkpoint_path, map_location="cpu").keys())
#     #     state_dict = torch.load(checkpoint_path, map_location="cpu")['state_dict']
#     #     self.load_state_dict(state_dict, strict=False)
