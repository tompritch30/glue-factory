...
for i in range(10, 30, 1):
    ...
    test_image_path_0 = fr"C:\Users\thoma\OneDrive\2023 Masters\Project\ProjectCode\external\glue-factory\tests\image_right\{imageIdx0:06d}_right.png"
    test_image_path_1 = fr"C:\Users\thoma\OneDrive\2023 Masters\Project\ProjectCode\external\glue-factory\tests\image_right\{imageIdx0:06d}_right.png"
    ...
    match_confidences = np.squeeze(pred["matching_scores0"], axis=0)

    # Print non-zero confidence values and count them
    non_zero_confidences = match_confidences[match_confidences > 0]
    for idx, conf in enumerate(non_zero_confidences):
        print(f"Non-zero confidence at match {idx}: {conf:.12f}")

    # Update totals
    total_matches += len(non_zero_confidences)
    confidences.extend(non_zero_confidences.astype(np.float64))
...
if confidences:
    ...
    min_conf = max_conf = avg_conf = 0

# Print results
print(f"Total number of matches: {total_matches}")
print(f"Minimum confidence: {min_conf:.12f}")
print(f"Maximum confidence: {max_conf:.12f}")
print(f"Average confidence: {avg_conf:.12f}")
